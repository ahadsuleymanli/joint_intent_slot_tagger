'''
    Credit: https://github.com/akoksal
    Source: https://github.com/akoksal/Turkish-Lemmatizer
'''
...
pickle_path = os.path.dirname(os.path.abspath(__file__))
pickle_path = os.path.join(pickle_path,'revisedDict.pkl')
...
def get_phrase_root(phrase):
    phrase = phrase.split(" ")
    if len(phrase)>1 and phrase[0] in ["bir","bi"]:
        phrase.pop(0)
    misin = ["mı","mi","mu","mü","mısın","misin","musun","müsün"]
    if len(phrase)>1 and phrase[-1] in misin:
        phrase.pop(-1)

    original = phrase[-1]

    phrase[-1] = get_lemma(phrase[-1])
